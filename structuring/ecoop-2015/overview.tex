...
procedure \code{span} (Figure~\ref{fig:span}) takes an argument $x$,
...
or~\code{null} if a successor doesn't exist.
...
constructed by the top-level call to \code{span} must actually be
...
% \begin{comment}
% An informal proof of the fact that \Code{span}, run from the root
% node, will return a spanning tree, would consider 4 cases according as
% $r_l$ and $r_r$ is \Code{true} or \Code{false}. If the corresponding
% recursive calls successfully marked some (possibly empty) sets of
% graph nodes, constructing the appropriate subtrees, then the enclosing
% call will construct the tree out of these subtrees by making $x$ its
% root and removing the links to the subgraphs, which are marked by the
% concurrent threads.
% %
% Furthermore, if \Code{span} succeeds, one must ensure that the marked
% subtree is \emph{maximal} \wrt the resulting \emph{final} graph's
% topology (\ie, that it cannot be extended by marking more reachable
% nodes) and that the nodes immediately reachable in the \emph{initial}
% graph from the constructed tree (\ie, the tree's \emph{front}) are all
% marked. Assuming that the initial graph is \emph{connected} and $z$ is
% its root, one can deduce that all nodes initially reachable from $z$
% are contained in the constructed tree, which is, thus, a spanning
% tree.

% \ab{Shouldn't there also be a proof that when \Code{span} returns
% \Code{true}, $x$ points to a tree and not a graph? Do we prove
% treeness? Also, can the informal proof be explained wrt. Fig. 2?}
% \end{comment}
...
it corresponds to the assigning \code{null} on lines~7 and~8 of
...
the pseudocode of the \code{span} procedure in Figure~\ref{fig:span}
into FCSL syntax.


% \begin{comment}
% By describing a concurroid for a graph with subjective auxiliary
% state, we defined an underlying local model of a spanning tree
% algorithm. We next tie this model to the actual program operations,
% such as \code{CAS} or pointer assignments. In FCSL, one does so by
% defining a number of \emph{atomic actions}---program operations,
% executed in one step and augmented with the protocol-specific logic,
% necessary to carry out the ultimate proof of correctness by reasoning
% about the joint state and the subjective views.

% One needs to prove that each action changing the state of the resource
% respects the transitions of the corresponding concurroid. For
% instance, the \code{CAS} on line~4 of Figure~\ref{fig:span} is
% underlied \ab{Unsure what underlied means here.} by a node-marking
% transition (in the case of success) and an \emph{identity} (\ie, idle)
% transition in the case of failure. Similarly, the assignments on
% lines~7 and~8 correspond to edge-removing transitions. This way, we
% can be sure that all modifications of the graph are correctly captured
% by the transitions of a corresponding concurroid. \ab{Can we give a
%   diagram here on how CAS changes state of resource? More generally,
%   can we give diagrams for all the atomic actions in Fig. 1? That
%   might help with the explanation.}
% %
% We will also need an action to read the contents of a graph node $x$
% on line~6. \ab{And line~4?} This action is trivially supported by an
% idle transition.
% %
% Finally, one needs to show that each action's outcome depends solely
% on its arguments and the state of the real heap, but not auxiliary
% resources. \ab{Say why this matters.}
% \end{comment}
...
\code{span_tp}. While Section~\ref{sec:devel} defines it formally and
