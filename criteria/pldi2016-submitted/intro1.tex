...
% \ab{General comment: Let's try to explain in words rather than give
%   derivations/proof outlines (cf Fig. 4); the derivations can go in
%   supplementary material. Let's also try to keep proof rules to a
%   minimum. Show them only if necessary, explain their uses in
%   examples. Call out the general pattern that we follow for both
%   examples early on. Auxiliary state, proof of implementation, proof
%   of client, hiding. Paper should not overwhelm non-experts (of which
%   there might be many) and experts (of which there might be 0 or very
%   few if we are lucky). Also make sure that we have incorporated POPL
%   rebuttal.}  
%
% \is{Okay, I think we addressed most (if not all) of this in the recent
% revisions.}
...
employ the specs in client verification.\ifext{}{\footnote{The proof
    scripts are available in supplementary material for the paper.}}
