...
\gad{I was wondering if it'd make sense to compare the effort needed
  to encode CAL/QCC/etc into FCSL with the previous one to reason with
  {linearizable} objects, i.e. your ESOP'15 paper?. If the
  ``countable'', e.g., lines of codes/time, aspects of them are
  similar, as I believe they should be, can we use this fact in our
  favor to stress the {\it off-the-shelf} argument? Can we use it to
  make a bigger delta from Hemed et.al's~\cite{Hemed-al:DISC15} work?}
%
\is{I'm not sure I understand what kind of strengthening you're 
  suggesting here by means of comparison to the previous work. Anyway,
  you're welcome to conduct such evaluation wrt. previous work and
  draw some conclusions from it, which we can then discuss and, if
  agreed, include into the paper.}
%
\gad{Let me expand a little bit. About stressing the {\it
    off-the-shelf} argument, my point was whether it is fair to say
  that encoding the verification of CAL/QCC. etc. into FCSL is
  ``similar'' to the effort done for encoding linearizable data
  structures done in the ESOP-15 paper, when measured in
  LOCs. In~\cite{Sergey-al:PLDI15} you guys crunched those numbers in
  the evaluation section and they look fairly similar. Furthermore,
  you made similar claims as to where the the biggest burden was. If
  you compare the ``sizes'', the concurroid/invariant parts are bigger
  here but in the same order of magnitude. This might be bullshit, as
  it ignores the inherent difference in complexity of the problems,
  which is uncountable, but my intention was to give some sort of
  measure ``to the you don't need a new logic (and the burden of its
  mechanization) if you have a good framework'' argument. As for the
  last part, I realize now that is not a good argument against Hemed in
  particular, but more of a general thing.}
